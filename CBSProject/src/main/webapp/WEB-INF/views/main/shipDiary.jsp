...
window.onload = ()=>{
	SDSel();
	// 각 행의 체크박스에 대한 클릭 이벤트 리스너 추가
    const checkboxes = document.querySelectorAll('.chkBox');
    checkboxes.forEach((checkbox) => {
        checkbox.addEventListener('click', () => {
            // 현재 클릭된 체크박스를 제외하고 모든 체크박스의 체크를 해제
...
const SDSel = () => {
	...
	xhr.open("GET", "/cbs/trnsprt/shipDiary", true);
   	...
   	// console.log("넘어온값 체크 : ", xhr.responseText);
    	...
    	let cnt = 1;
  		for (var i = 0; i < res.length; i++) {
		    str += "<tr onclick='selectRow(this)'>";
		    str += "<td class='text-center'><input class='form-check-input chkBox' type='checkbox'></td>";
		    str += "<td>" + cnt + "</td>";
		    str += "<td>" + res[i].blNo + "</td>";
		    str += "<td>" + res[i].startPrt + "</td>";
		    str += "<td>" + res[i].arvlPrt + "</td>";
		    str += "<td>" + res[i].shipStartDe + "</td>";
		    str += "<td>" + res[i].shipEndDe + "</td>";
		    str += "<td>" + res[i].shipNm + "</td>";
		    str += "<td>" + res[i].shipDest + "</td>";
	    	str += "</tr>";
	    	cnt++;
    	}
    	qs("#SDTab").innerHTML = str;
	};
	xhr.send();
};
...
const shipChoice = ()=>{
	...
	const chk = chkBoxes[0].parentNode.parentNode
	const selectedVal = [];
	//console.log(chk)
	let data = {};
	chkBoxes.forEach((checkbox) => {
		const row = checkbox.closest('tr'); // 체크박스의 부모 tr 요소 찾기
		const columns = row.querySelectorAll('td'); // tr 요소의 모든 td 요소 가져오기
		const rowData = [];
		
		columns.forEach((column) => {
		    if (column !== checkbox.parentNode) { // 체크박스의 부모 td는 제외
		        rowData.push(column.textContent); // td 요소의 텍스트 내용 가져와서 rowData 배열에 추가
		    }
		});
		selectedVal.push(rowData); // rowData 배열을 selectedValues 배열에 추가
	});
	console.log(selectedVal);
	for(let i=0; i<selectedVal.length; i++){
		let row = selectedVal[i];
		const blNo = row[1];
		const startPrt = row[2];
		const arvlPrt = row[3];
		const shipStartDe = row[4];
		const shipEndDe = row[5];
		const shipNm = row[6];
		const shipDest = row[7];
		data = {
				blNo:blNo,
				startPrt:startPrt,
				arvlPrt:arvlPrt,
	...
	//window.opener : 부모창
	window.opener.document.getElementById("trnsprtStle").value = data.startPrt;	
	window.opener.document.getElementById("waybilNo").value = data.blNo;	
	window.opener.document.getElementById("startPrt").value = data.startPrt;	
	window.opener.document.getElementById("arvlPrt").value = data.arvlPrt;	
	window.opener.document.getElementById("shipStartDe").value = data.shipStartDe;	
	window.opener.document.getElementById("shipEndDe").value = data.shipEndDe;	
	window.opener.document.getElementById("shipNm").value = data.shipNm;	
	window.opener.document.getElementById("coudes").value = data.shipDest;	
...
<div class="row" style="width: 100%;">
	...
	<div class="card p-4" style="width: 100%; height: 700px;">
			<div>
				<h3 class="card-title p-3" style="float: left;">선박 운항 일지</h3>
				<button class="btn btn-primary me-1 mb-1" id="btnInsert" onclick="shipChoice()" style="float: right; margin-top: 10px;">선택</button>
			</div>
		    <table class="table table-hover" style="text-align: center;">
		    	...
		    	<tbody class="list" id="SDTab"></tbody>
		    </table>
		</div>
