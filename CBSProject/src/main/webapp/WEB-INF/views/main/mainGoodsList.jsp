...
const searchText = function(){
	...
	mainGoodsList(searchText);
}

/*상품 목록 가져오는 함수*/
const mainGoodsList = function(searchText){
	var xhr = new XMLHttpRequest();
	xhr.open("POST", "/cbs/cstbrkr/GoodsSearch", true);
	xhr.onreadystatechange = function(){
        if(xhr.readyState == 4 && xhr.status == 200){
            var response = JSON.parse(xhr.responseText);
            console.log("상품 가져온 값 : ", response);
            mainGoodsTbodyAdd(response);
            mainGoodsClick(response);
        }
    }
    xhr.send(searchText);
}
...
/*상품 목록 행 추가하는 함수*/
const mainGoodsTbodyAdd = function(response){		
	var str = "";
	response.forEach(function(mainGoodsVO, idx){
		console.log("goods["+idx+"]", mainGoodsVO);
		str += "<tr>";
		str += "<td><input type='checkbox' id='checkbox"+idx+"' onclick='handleCheckboxClick("+idx+")'/></td>";
        str += "<td>"+(idx+1)+			"</td>";
        str += "<td>"+mainGoodsVO.hsCd+		"</td>";
        str += "<td>"+mainGoodsVO.goodsCl+	"</td>";
        str += "<td>"+mainGoodsVO.goodsNo+	"</td>";
        str += "<td>"+mainGoodsVO.goodsNm+	"</td>";
        str += "</tr>";
	});
	document.querySelector("#mainGoodsTbody").innerHTML = str;
}

const handleCheckboxClick = function(idx) {
    var checkboxes = document.querySelectorAll('.mainGoodsList input[type="checkbox"]');
...
var selectedGoods ={};
const mainGoodsClick = function(response){		
	console.log("클릭되고 받아오는 response 값 : ", response);
	document.querySelectorAll('.mainGoodsList tr').forEach(function(row, idx) {
	    row.addEventListener('click', function() {
	        // 모든 체크박스 해제.
	        document.querySelectorAll('.mainGoodsList input[type="checkbox"]').forEach(function(checkbox) {
	            checkbox.checked = false;
	        });
	        // 클릭한 행의 체크박스 체크
	        ...
	        console.log("클릭한 행의 정보 :", selectedRowData);
	        
	        selectedGoods = {
	                goodsNo: selectedRowData.goodsNo,
	                goodsNm: selectedRowData.goodsNm,
	                hsCd: selectedRowData.hsCd
	        };
	        console.log("클릭한 행의 정보 포장한 JSON : ", selectedGoods);
	    });
	});
}

const mainGoodsSelect = function(){
// 	if(selectedGoods==null){
// 		console.log("데이터 안들어가있음");
// 		return;
// 	}else{
// 		window.opener : 부모창
		window.opener.document.getElementById("hsCd").value = selectedGoods.hsCd;	
		window.opener.document.getElementById("goodsNo").value = selectedGoods.goodsNo;	
		window.opener.document.getElementById("goodsNm").value = selectedGoods.goodsNm;	
...
<div class="row" style="width: 100%;">
	<div>
		<div class="card p-4" style="width: 100%; height: 700px;">
			<div>
				<h3 class="card-title p-3" style="float: left; margin-top: 10px;">상품 목록</h3>
				<div class="input-group mb-3 w-50 ms-auto p-2 bd-highlight" style="padding: 10px;">
				    <input type="text" id="searchText" class="form-control" placeholder="검색어를 입력하세요." aria-label="" aria-describedby="button-addon2" style="margin-top: 10px;">
				    <button class="btn btn-primary btn-sm" type="button" id="search" onclick="searchText()" style="margin-top: 10px;">검색</button>
					<button class="btn btn-phoenix-primary btn-sm px-3 me-1" id="btnInsert" onclick="mainGoodsSelect()" style="float: right; margin-top: 10px;">선택</button>
				</div>
			</div>
			<div class="tableDiv">
			    <table class="table table-sm fs-9 mb-0" style="text-align: center;">
			    	...
			    	<tbody class="mainGoodsList" id="mainGoodsTbody">
			    		<!-- 물품 추가되는 곳 -->
			    	</tbody>
			    </table>
			</div>
