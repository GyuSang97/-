...
public class CstbrkrServiceImpl implements CstbrkrService {
	...
	public int insertEntryThings(List<EntryThingVO> entryThingVOList) {
		...
		for (EntryThingVO entryThingVO : entryThingVOList) {
			...
			hsCodeAmount += totAmount * goodsTaxrt;
		...
		for (int i = 1; i <= 10; i++) {
			...
			case 3:
				...
				double transportationEnergyEnvironmentTax = totalTaxAmount * ((4.1 / 9.3) * 375);
			...
			case 7:
				...
				double vat = (hsCodeAmount + (totalTaxAmount * 0.05) + (totalTaxAmount * ((4.1 / 9.3) * 375)) + (0)
